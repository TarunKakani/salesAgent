...
def total_sales_product(
    product: str,
) -> float:  # this syntax is called type hints, better code better readiblity
...
def compare_total_sales_q1_q2(product) -> Tuple[float, float]:
    q1 = current_df[
        (current_df["Date"].dt.quarter == 1) & (current_df["Product"] == product)
    ]["Total_Sales"].sum()
    q2 = current_df[
        (current_df["Date"].dt.quarter == 2) & (current_df["Product"] == product)
    ]["Total_Sales"].sum()
    return q1, q2
...
def biggest_percent_increase() -> Tuple[str, float]:
    ...
    pct = (
        (curr - hist) / hist.replace(0, math.nan)
    ).dropna()  # replace zero values with nan(null) and drop those values (delete rows)
    top = pct.idxmax()
...
def ask_low(query: str) -> str:
    ...
    # not working
        ...
        return (
            f"Average price for {product} = ${avg_price:.2f}"
            if not math.isnan(avg_price)
            else f"No data for {product}"
        )
...
MODEL_PATH = "/Users/watchdog/Documents/salesAgent/models/Phi-3-mini-4k-instruct-q4.gguf"  # Model Path
